...
def publish_move(data):
    jointpub = rospy.Publisher('/om_with_tb3/joint_trajectory_point',Float64MultiArray,queue_size =10, latch=True)
    rospy.Rate(1).sleep()
...
def publish_move_gripper(data):
    jointpub = rospy.Publisher('/om_with_tb3/gripper_position',Float64MultiArray,queue_size =10, latch=True)
    rospy.Rate(1).sleep()
    joint_pos = Float64MultiArray()
...
def exeGripper(destination):
    rate = rospy.Rate(20)
    read_joint_states()
    print("Currently at: {}".format(np.array(currentGripper)))
    print("want to go to: {}".format(np.array(destination)))
    publish_move(np.array(destination))
...
def move_arm():    
    moveTo(destination=[-0.5,-0.5,0,0,0])
    #go back

    rospy.logwarn("Trying array values now")
    publish_move_gripper([-0.01])
    rospy.Rate(1).sleep()
    ...
    moveTo(destination=[-0.5,0,0,0,0])
    publish_move_gripper([0.005])
    rospy.Rate(1).sleep()


'''
    moveTo(destination=[-0.5,0.15,1.25,-0.96,0.7])
    #turn towards shelf
    moveTo(destination=[-0.5,-1.48,1.22,-1.04,-0.3])
    #lean in
    moveTo(destination=[-0.5,-1.48, -0.12,0.26,-0.4])
    #lean back out
    moveTo(destination=[-0.5,-1.48,1.17,-0.8,-0.3])
 


    moveTo(destination=[-0.5,-1.61,1.04,-0.65,-0.75])

    moveTo(destination=[-0.5,-3.08,1.25,-0.82,-0.75]) 

    moveTo(destination=[-0.5,-3.08,1.04,-0.5,-0.12])

    moveTo(destination=[-0.5,-3.08,0.16,-0.16,-0.12])

    moveTo(destination=[-0.5,-3.01,1.12,-1.2,0.12])

    moveTo(destination=[-0.5,0.02,1.2,-1.25,0.1])
